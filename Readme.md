...
リアルタイム打刻＋手動入力に対応し、月次でプロジェクトごとの稼働時間と請求額（時給 × 稼働時間、100円未満切り捨て）を確認できます。
...
- 請求先マスタ・請求書 PDF（内税/外税、プロジェクト別稼働明細）
- **請求書の任意明細**（プロジェクト × 月ごとの項目名・金額。時給換算とは別行で PDF に反映）
...
- `specs/07-invoicing.md` 請求書 PDF・任意明細・税計算
